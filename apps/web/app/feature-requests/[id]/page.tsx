...
export default async function FeatureRequestDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ workspaceId?: string }> }) {
  ...
  return <main style={mainStyle}>
    ...
    <Card style={{ marginTop: 16 }}><h2>PRD</h2><p style={mutedStyle}>Generate, edit, approve, or regenerate the product requirements document before tasks can be created.</p><Link href={`/feature-requests/${id}/prd${query.workspaceId ? `?workspaceId=${query.workspaceId}` : ""}`} style={buttonStyle}>Open PRD editor</Link></Card>
